...
class Pointer {
        ...
        for (const key of Object.keys(data)) {
            ...
            if (fieldDescription.type === 'StringType') {
                ...
                if (fieldDescription.primaryKey) {
                    const tableData = JSON.parse(await fs.readFile(join(__dirname, '.', 'data', `${this.currentTable}.json`), 'utf-8'));
                    if (tableData.data.map((item: any) => item[key]).indexOf(value) >= 0) {
    ...
    async checkForeignKey(otherTable: string, fieldValue: any, fieldName: string) {
        ...
        if (await exists(pathToOtherTableName)) {
            ...
            if (Object.keys(otherTableFileDataJson.fields).indexOf(fieldName) === -1) {
                ...
                if (otherTableFileDataJson.data.map((item: any) => item[fieldName]).indexOf(fieldValue) <= -1) {
    ...
    async get(filter: any, tableName = this.currentTable) {
        ...
        return tableData.data.filter((object: any) => objectsToSend.indexOf(object[primaryKeyField]) !== -1);
    }

    async update(pkValue: any, data: any, tableName = this.currentTable) {
        if (this.isConnected !== true) {
            throw new Error('You must connect to the table first.');
        }
        data = await this.validate(data);
    ...
    async deleteObject(pkValue: any, tableName = this.currentTable) {
        if (this.isConnected !== true) {
            throw new Error('You must connect to the table first.');
        }
        const primaryKeyField = await this.getPrimaryKeyFieldOfTable(tableName);
            ...
            for (const reference of tableData.references) {
                ...
                switch (otherTableData.fields[fkName].onDelete) {
                    case 'Cascade':
                        ...
                        const dataIndexCascade = tableData.data.findIndex((item: any) => item[primaryKeyField] === pkValue);
                    ...
                    case 'SetNull':
                        // Удаляем запись из основной таблицы
                        const dataIndex = tableData.data.findIndex((item: any) => item[primaryKeyField] === pkValue);
                        if (dataIndex !== -1) {
    ...
    async filterObjects(filter: any, tableName: string) {
        ...
        const filteredObjects = tableData.data.filter((object: any) => {
        ...
        const primaryKeyValues = filteredObjects.map((object: any) => object[primaryKeyField]);
